	...
	T* oldBuffer, int32 oldCapacity, int32 newCapacity)
{
	...
	if (oldBuffer)
	{
		memcpy(static_cast<void*>(newBuffer), oldBuffer, sizeof(T) * oldCapacity);
...
template <typename T> T* b2ParticleSystem::RequestBuffer(T* buffer)
{
	if (!buffer)
	{
		...
		b2Assert(buffer);
		memset(static_cast<void*>(buffer), 0, sizeof(T) * m_internalAllocatedCapacity);
	}
	return buffer;
}
...
inline void b2ParticleSystem::PrepareForceBuffer()
{
	if (!m_hasForce)
	{
		memset(static_cast<void*>(m_forceBuffer), 0, sizeof(*m_forceBuffer) * m_count);
		m_hasForce = true;
	}
}
